...
func (suite *ServiceTestSuite) TestNewTransaction() {
	...
	for _, item := range data {
		gomock.InOrder(
			suite.mockTransactionRepository.EXPECT().FindOperationTypeById(gomock.Any()).Return(item.operationTypeFound, item.errorToReturn),
			suite.mockTransactionRepository.EXPECT().NewTransaction(gomock.Any()).Return(item.expectedTransaction, item.expectedError),
		)
		suite.transactionService = transaction.NewTransactionService(suite.mockTransactionRepository, &transaction.Helper{})
