...
func (suite *ControllerTestSuite) TestNewAccount() {
	...
	data := []struct {
	...
	}{
		{
			...
			method:           "POST",
			bodyData:         "{\"document\": \"12121212\"}",
			expectedCode:     http.StatusCreated,
			responseExpected: fmt.Sprintf("{\"status\":%v,\"data\":{\"id\":\"%v\",\"document\":\"%v\"}}", http.StatusCreated, objectID.Hex(), "12121212"),
			expectedError:    nil,
		},
		{
			accountCreated:   nil,
...
func (suite *ControllerTestSuite) TestFindByID() {
	...
	data := []struct {
	...
	}{
		{
			...
			responseExpected: fmt.Sprintf("{\"status\":%v,\"data\":{\"id\":\"%v\",\"document\":\"%v\"}}", http.StatusOK, objectID.Hex(), "12121212"),
			expectedError:    nil,
		},
		{
			expectedAccount:  nil,
